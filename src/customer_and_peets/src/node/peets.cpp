...
class peetsNode : public rclcpp::Node
{
public:
    // 构造函数,第一个参数为节点名称, 并初始化count为1
    peetsNode(std::string name) : Node(name), count(1)
    {
        // 打印KFC的自我介绍
        ...
        advertisement_timer = this->create_wall_timer(5000ms, std::bind(&peetsNode::advertisement_timer_callback, this));
